                ...
                contents.map((content) => (
                    <button key={content.id} onClick={e => ContentsHandler(e, content.contentType)} value={content.id} className="w-full text-left">
                        <ul className='justify-start mb-4 text-black pl-1'>
                            <div className="flex">
                                {/* <li key={content.type} className="flex justify-start mt-6 mr-1">
                                <div className='flex justify-center items-center h-6 text-orange mr-2 text-center text-md font-medium rounded-xl'>{content.contentType === "scholarship" ? "장학금" : (content.contentType === "activity" ? "대외활동" : "공모전")}</div>
                            </li> */}
